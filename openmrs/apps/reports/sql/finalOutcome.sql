SELECT 	pi.identifier, 
  ...
  JOIN (select cast(COALESCE(orders.scheduled_date, orders.date_activated) AS date) AS start_date,orders.encounter_id,orders.order_id,orders.voided from orders where voided = 0) orders
