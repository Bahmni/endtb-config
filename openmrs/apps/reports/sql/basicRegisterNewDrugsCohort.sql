SELECT  ppa.value_reference AS `Registration Number`,
        person_name.family_name AS `Last name`,
        person_name.given_name AS `First name`,
        ...
        MAX(IF(obs.concept_full_name = 'Baseline, Drug resistance', obs.value, NULL)) AS `DR Resitance profile`,
        ...
        MAX(IF(date_obs.concept_full_name='TUBERCULOSIS DRUG TREATMENT START DATE',TIMESTAMPDIFF(DAY,date_obs.date_value, NOW())/30,NULL))          AS `treatment duration`,
        MAX(IF(dd.name = 'Delamanid (Dlm)',DATE_FORMAT(dd.start_date, '%d/%b/%Y'),NULL )) AS `Dlm Start Date`,
        MAX(IF(dd.name = 'Delamanid (Dlm)',dd.duration,NULL )) AS `Dlm Duration`,
        MAX(IF(dd.name = 'Bedaquiline (Bdq)',DATE_FORMAT(dd.start_date, '%d/%b/%Y'),NULL )) AS `Bdq Start Date`,
        MAX(IF(dd.name = 'Bedaquiline (Bdq)',dd.duration,NULL )) AS `Bdq Duration`,
  cf.facility,
  ...
  regimen.ImpCln   ,
  regimen.`Amx-Clv`,
        MAX(IF(obs.concept_full_name = 'Baseline, HIV serostatus result', obs.value, NULL)) AS `HIV baseline HIV`,
        MAX(IF(add_more_obs.concept_full_name = 'Lab, HIV test result', add_more_obs.value, NULL)) AS `Lab, HIV test result`,
        MAX(IF(obs.concept_full_name = 'Baseline, Hepatitis B', obs.value, NULL)) AS `Baseline, Hepatitis B`,
        MAX(IF(add_more_obs.concept_full_name = 'Lab, Hepatitis B antigen test result', add_more_obs.value, NULL))  AS `Lab, Hepatitis B antigen test result`,
        MAX(IF(obs.concept_full_name = 'Baseline, Hepatitis C', obs.value, NULL))                                   AS `Baseline, Hepatitis C`,
        MAX(IF(add_more_obs.concept_full_name = 'Lab, Hepatitis C antibody test result', add_more_obs.value, NULL)) AS `Lab, Hepatitis C antibody test result`,
        MAX(IF(obs.concept_full_name = 'Diabetes Mellitus', obs.value, NULL))                                       AS `Baseline, Diabetes`,
        MAX(IF(obs.concept_full_name = 'EOT, Outcome', obs.value, NULL))                                            AS `EOT, Outcome`,
        MAX(IF(date_obs.concept_full_name='Tuberculosis treatment end date',DATE_FORMAT(date_obs.date_value, '%d/%b/%Y'),NULL))              AS `End of treatment`,
