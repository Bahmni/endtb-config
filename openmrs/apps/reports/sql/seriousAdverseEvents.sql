SELECT
  ...
  GROUP_CONCAT(DISTINCT (IF(first_child_cn.name = 'SAE Form, Date event became serious', DATE_FORMAT(first_child_obs.value_datetime, '%d-%M-%Y'), NULL)) SEPARATOR ',') AS 'Date event became serious',
...
FROM obs top_level_obs
  INNER JOIN patient p ON p.patient_id = top_level_obs.person_id AND p.voided = 0
