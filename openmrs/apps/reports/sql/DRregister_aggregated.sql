SELECT MAX(IF(pat.name='Registration Number', ppa.value_reference, NULL )) AS `Registration Number`,
      ...
      regimen.`Amx-Clv`,
       ...
       COALESCE(MAX(IF(add_more_obs.concept_full_name = 'Lab, HIV test result', add_more_obs.value, NULL)),MAX(IF(obs.concept_full_name = 'Baseline, HIV serostatus result', obs.value, NULL))) AS `HIV result`,       
...
FROM
  ...
  LEFT JOIN (
    SELECT  COALESCE(answer_concept.concept_short_name,answer_concept.concept_full_name,o.value_datetime,o.value_numeric,o.value_text) AS value, ee.episode_id,cv.concept_full_name
     ...
     WHERE cv.concept_full_name IN ('Lab, HIV test result',
           ...
           AND o.obs_datetime = (SELECT MAX(obs_datetime) FROM obs ,episode_encounter
  ...
  LEFT JOIN (
     ...
     FROM obs obs1
       ...
       LEFT JOIN (SELECT o.obs_datetime, o.obs_id, o.value_datetime, ee.episode_id
                  FROM obs o
  ...
  LEFT JOIN (
    SELECT COALESCE(o.value_datetime, o.obs_datetime) AS end_of_treatment_date, ee.episode_id
      FROM obs o, concept_view cv, episode_encounter ee
