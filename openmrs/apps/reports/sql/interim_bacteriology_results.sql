SELECT
  ...
  COALESCE(MAX(CASE WHEN bacteriology_results.result = 'Positive for M. tuberculosis'
   THEN 'Positive for M. tuberculosis' END),
   MAX(CASE WHEN bacteriology_results.result = 'Negative for M. tuberculosis'
     THEN 'Negative for M. tuberculosis' END),
   MAX(CASE WHEN bacteriology_results.result = 'Contaminated'
     THEN 'Contaminated' END),
   MAX(CASE WHEN bacteriology_results.result = 'Only positive for other mycobacterium'
     THEN 'Only positive for other mycobacterium' END),
   MAX(CASE WHEN bacteriology_results.result = 'Other'
     THEN 'Other' END), NULL) AS bacteriology_result,
  CAST(GREATEST(DATEDIFF(COALESCE(MAX(CASE WHEN bacteriology_results.result = 'Positive for M. tuberculosis'
    THEN obs_datetime END),
    MAX(CASE WHEN bacteriology_results.result = 'Negative for M. tuberculosis'
      THEN obs_datetime END),
    MAX(CASE WHEN bacteriology_results.result = 'Contaminated'
      THEN obs_datetime END),
    MAX(CASE WHEN bacteriology_results.result = 'Only positive for other mycobacterium'
      THEN obs_datetime END),
    MAX(CASE WHEN bacteriology_results.result = 'Other'
      THEN obs_datetime END), NULL), drug_start_date), 0) AS UNSIGNED) AS numberOfDays
FROM (SELECT
  ...
  (SELECT
   ...
   FROM obs
    ...
    ON bacteriology_results.patient_program_id = interim_outcome_results.patient_program_id
GROUP BY interim_outcome_results.patient_program_id;
