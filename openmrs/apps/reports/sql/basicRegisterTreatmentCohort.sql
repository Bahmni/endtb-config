SELECT  ppa.value_reference AS `Registration Number`,
       ...
       TRUNCATE((TIMESTAMPDIFF(DAY,tStartDate.value_datetime, NOW())/30),1) AS `treatment duration`,
  ...
  regimen.`Amx-Clv`,
       ...
       MAX(IF(add_more_obs.concept_full_name = 'Lab, Hepatitis B antigen test result', add_more_obs.value, NULL)) AS `Hep B baseline`,
