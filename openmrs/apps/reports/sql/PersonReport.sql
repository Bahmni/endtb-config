SELECT
  ...
  GROUP_CONCAT(DISTINCT(IF(pat.name = 'nationalIdentificationNumber', CONCAT('\"',o.attr_value,'\"'), NULL)) SEPARATOR ',') AS `id_nat`,
  DATE_FORMAT(o.date_created, '%d/%b/%Y') as `creation_date`,
  GROUP_CONCAT(DISTINCT(IF(pat.name = 'patientAddress',CONCAT('\"',o.attr_value,'\"'), NULL)) SEPARATOR ',') AS `address_line1`,
  GROUP_CONCAT(DISTINCT(IF(pat.name = 'patientAddressLine2',CONCAT('\"',o.attr_value,'\"'), NULL)) SEPARATOR ',') AS `address_line2`,
  GROUP_CONCAT(DISTINCT(IF(pat.name = 'patientDistrict',CONCAT('\"',o.attr_value,'\"'), NULL)) SEPARATOR ',') AS `res_district`,
  GROUP_CONCAT(DISTINCT(IF(pat.name = 'patientCountry',CONCAT('\"',o.attr_value,'\"'), NULL)) SEPARATOR ',') AS `res_country`,
...
FROM
  (SELECT
     ...
     CONCAT('\"',pn.given_name,'\"') as given_name,
     CONCAT('\"',pn.family_name, '\"') as family_name,
