...
VALUES ('endtb.sqlSearch.dataIntegrity',
        ...
        FROM
          ...
          JOIN patient_program_attribute ppa2 ON pp.patient_program_id = ppa2.patient_program_id AND ppa2.voided=0
          JOIN program_attribute_type pat2  ON  pat2.name=''Registration Facility''
          LEFT JOIN concept_view cv_reg ON cv_reg.concept_id = ppa2.value_reference AND ppa2.attribute_type_id = pat2.program_attribute_type_id
          LEFT JOIN obs treat_det ON treat_det.concept_id= max_date.concept_id  AND treat_det.voided=0 AND treat_det.encounter_id = ee.encounter_id
                                     AND (epp.episode_id, treat_det.obs_datetime) IN (SELECT max_date_ee.episode_id,
        ...
        WHERE pat.name = ''Registration Number'' AND p.voided = 0 AND pn.voided = 0
          GROUP BY DQ_COLUMN_TITLE_RULE_NAME, ppa.value_reference, epp.episode_id
